...
namespace WinApp.Code
{
	class GridSortingHelper
	{
		...
		public async static Task<Sorting> GetSorting(GridFilter.Settings currentGridFilter)
		{
			...
			if (dt.Rows.Count > 0)
			{
				if (dt.Rows[0]["lastSortColumn"] != DBNull.Value)
				{
					...
					ColListHelper.ColListItem clc = await ColListHelper.GetColListItem(sorting.ColumnHeader, MainSettings.View);
