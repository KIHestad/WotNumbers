...
{
	public partial class ucGaugeRWR : UserControl
	{
        string _battleMode = "";
        private GadgetHelper.TimeRangeEnum _battleTimeSpan = GadgetHelper.TimeRangeEnum.Total;

        public ucGaugeRWR(string battleMode, GadgetHelper.TimeRangeEnum timeSpan)
		{
			InitializeComponent();
		...
		public void DataBind()
		{
			...
			for (byte i = 0; i <= 8; i++)
			{
				...
				if (i == 0)
					...
					aGauge1.RangesStartValue[i] = (float)ColorRangeScheme.RangeRWR[i];
				if (i == 8)
					aGauge1.RangesEndValue[i] = aGauge1.ValueMax;
				else
                    aGauge1.RangesEndValue[i] = (float)ColorRangeScheme.RangeRWR[i + 1];
				aGauge1.RangeEnabled = true;
			}
            // Show battle mode
            string capText = "Total";
            capText = BattleMode.GetItemFromSqlName(_battleMode).Name;
            aGauge1.CenterSubText = "RWR: " + capText;
			// Overall stats team
            ...
            aGauge1.CenterTextColor = ColorRangeScheme.RWRcolor(end_val);
