...
_VEH_CELL_RESULTS_PUBLIC = Meta((
 ...
 'goals', int, 0, None, 'sum'), (
 'selfGoals', int, 0, None, 'sum'), (
 'assists', int, 0, None, 'sum'), (
 'steals', int, 0, None, 'sum'), (
 'blocks', int, 0, None, 'sum'), (
 'productivityPoints', int, 0, None, 'sum'))
...
class VehicleInteractionDetails(object):
    ...
    def toDict(self):
        return dict(
			[ ((vehID, vehIdx), dict(_VehicleInteractionDetailsItem(self.__values, offset))) for (vehID, vehIdx), offset 
				in self.__offsets.iteritems()
            ])